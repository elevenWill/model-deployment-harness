...
官方推荐配置是高质量基线，不是最低硬门禁。目标硬件未出现在推荐配置中时，应先产出
`CompatibilityAssessment` 并进入 `RESEARCH_NEEDED`，检索官方资料、Issue/PR、论坛和技术社区中的
候选适配方案。社区内容只能停留在 `COMMUNITY_LEAD`/`RESEARCH_NEEDED`，不能直接进入容量试跑、规划或执行；候选方案必须有 S/A 级上游证据逐一关联具体缺口与缓解机制并覆盖全部缺口、满足
目标主机条件并绑定最新只读主机观测。尚未复现时进入 `READY_FOR_TRIAL`，先生成一份独立序列化、审核
为 `READY` 且 `purpose=CAPACITY_TRIAL` 的容量试跑计划；它仍经过正常执行与 L5/L6 验证。请求、主机
档案、试跑计划、完整步骤执行记录、InferenceProof、原始请求/响应/任务/输出、SemanticReview、
验证结果和输出制品的哈希及交叉引用全部一致，并经可信媒体校验重建 L5/L6 后，适配状态才是
`VALIDATED`。这只允许把适配证据作为正式计划输入，不代表完整需求、许可、证据或计划审核已经通过，
更不直接授权远程写入。调研结束后无候选方案、条件不匹配或容量试跑失败才 `BLOCKED`。许可、安全、真实物理容量、
受保护变更和无法完成必要验证等不可绕过门禁始终 `BLOCKED`。

运行环境支持 subagent 时，适配调研和候选修复应交给独立 subagent；主会话只接收结构化证据、候选
方案、复现结论和计划条件，不回填完整搜索过程。调研 subagent 没有远程写入权限，修复或复现需要写入
时仍必须等待独立审核后的精确计划，并遵守单一写入者约束。
