...
禁止的转换包括 `INTAKE -> EXECUTE`、`RESEARCH -> EXECUTE` 与 `HOST_DISCOVERY -> EXECUTE`。门禁失败时必须以 `NEEDS_USER_INPUT` 或 `BLOCKED` 停止推进。

## 需求门禁

用户意图与主机事实是不同的数据类别。

- 必需用户意图包括目标主机选择器及 SSH 用户名/端口、精确 GPU ID、安装根目录、模型根目录、模型及变体、框架选择或在支持框架中选择的权限、服务模式、绑定主机、端口，以及是否可修改现有环境。以 schema 和 policy 中的规范清单为准。绝不能猜测或默认填充缺失的必需意图：输出 `NEEDS_USER_INPUT`，列出缺失字段，并按照 policy 在发现或执行前停止。
